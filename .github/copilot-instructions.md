...
11. **Page headers use gradient style** — `.master-data-header` in `master-data.css` uses purple gradient with ALL children forced white via `!important`. Use `data-theme` attribute for module-specific colors: `teal` (journal/accounting), `blue`, `emerald`, `rose`, `amber`. Button classes `.btn-header-primary` (translucent white) and `.btn-header-outline` (subtle white border) are defined globally — do NOT duplicate in views or use inline styles on header elements.
12. **Standalone PDF views need `global $config;`** — When a PDF view (e.g., `delivery/print.php`) is `include`d from a controller, `$config` from `sys.configs.php` is not in scope. Always add `global $config;` before the `if (!isset($config))` fallback block. Without this, cPanel deployment fails with `getaddrinfo for mysql failed` because the fallback hardcodes Docker's `mysql` hostname.
13. **Use `?:` not `??` for empty-string fallbacks** — `$row['field'] ?? 'default'` only catches NULL, not empty string `''`. For DB fields that may be empty (e.g., `company.name_sh`), use `trim($row['field'] ?? '') ?: 'default'` to also handle empty strings. This caused blank serial numbers on cPanel where `name_sh` was `''` instead of NULL.
