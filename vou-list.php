...
<?php
$query=mysqli_query($db->conn, "select voucher.id as id,voucher.name as name, voucher.email as email,phone, DATE_FORMAT(createdate,'%d-%m-%Y') as createdate, voucher.description as description,vou_rw,voucher.brand as brand,voucher.vender as vender from voucher  where  voucher.vender='".$_SESSION['com_id']."' order by voucher.id desc");
