...
<?php
$query=mysqli_query($db->conn, "select po.id as id,	countmailinv, po.name as name, taxrw as tax,status_iv,  DATE_FORMAT(valid_pay,'%d-%m-%Y') as valid_pay, name_en, DATE_FORMAT(deliver_date,'%d-%m-%Y') as deliver_date ,status from po join pr on po.ref=pr.id join company on pr.cus_id=company.id join iv on po.id=iv.tex where  po_id_new='' and pr.ven_id='".$_SESSION['com_id']."' and status>='4' order by iv.id desc ");
$cot=0;
...
<?php
$query=mysqli_query($db->conn, "select po.id as id, po.name as name,  taxrw as tax,  DATE_FORMAT(valid_pay,'%d-%m-%Y') as valid_pay, name_en, DATE_FORMAT(deliver_date,'%d-%m-%Y') as deliver_date ,status from po join pr on po.ref=pr.id join company on pr.ven_id=company.id join iv on po.id=iv.tex where  po_id_new='' and pr.cus_id='".$_SESSION['com_id']."' and status>='4'  order by iv.id desc ");
$cot=0;
