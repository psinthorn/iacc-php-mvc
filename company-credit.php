...
$query=mysqli_query($db->conn, "select * from company_credit where id='".$id."'");
...
<form action="core-function" method="post" id="myform">
	<div id="box">
		...
		<select id="cus_id" name="cus_id" class="form-control">
			<?php $querycustomer=mysqli_query($db->conn, "select name_en,id from company where id not in (select cus_id from company_credit where ven_id='".$ven_id."' group by cus_id) and id!='".$ven_id."' and customer='1' ");
