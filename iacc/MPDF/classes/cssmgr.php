...
function MergeCSS($inherit,$tag,$attr) {
	...
	if (!isset($attr['ID'])) { $attr['ID'] = ''; } else { $attr['ID'] = (string)$attr['ID']; }
	//===============================================
/*-- TABLES --*/
...
/*-- END LISTS --*/
	...
	if ($inherit == 'BLOCK') {
		...
		//===============================================
		// Save Cascading CSS e.g. "div.topic p" at this block level
		$this->_mergeFullCSS($this->cascadeCSS, $this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS'], $tag, $classes, (string)$attr['ID']);
		//===============================================
		// Cascading forward CSS
		//===============================================
		$this->_mergeFullCSS($this->mpdf->blk[$this->mpdf->blklvl-1]['cascadeCSS'], $this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS'], $tag, $classes, (string)$attr['ID']);
		//===============================================
		  // Block properties
...
/*-- END TABLES --*/
	...
	if ($inherit == 'BLOCK') {
		...
		$this->_set_mergedCSS($this->mpdf->blk[$this->mpdf->blklvl-1]['cascadeCSS']['ID>>'.((string)$attr['ID'])], $p);
		foreach($classes AS $class) {
			$this->_set_mergedCSS($this->mpdf->blk[$this->mpdf->blklvl-1]['cascadeCSS'][$tag.'>>CLASS>>'.$class], $p);
		}
		$this->_set_mergedCSS($this->mpdf->blk[$this->mpdf->blklvl-1]['cascadeCSS'][$tag.'>>ID>>'.((string)$attr['ID'])], $p);
	}
		...
		$this->_set_mergedCSS($this->mpdf->blk[$this->mpdf->blklvl]['cascadeCSS']['ID>>'.((string)$attr['ID'])], $p);
		foreach($classes AS $class) {
