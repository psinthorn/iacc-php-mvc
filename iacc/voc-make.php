...
$queryvou=mysqli_query($db->conn, "select * from voucher where id='" . mysqli_real_escape_string($db->conn, $_REQUEST['id'] ?? '') . "' and vender='" . mysqli_real_escape_string($db->conn, $_SESSION['com_id'] ?? '') . "'");
...
<form action="core-function.php" method="post" id="company-form">
    ...
    <div id="box">
		...
		<select id="brandven" name="brandven" class="form-control">
			...
			$querycustomer=mysqli_query($db->conn, "select brand_name,id from brand where ven_id='" . mysqli_real_escape_string($db->conn, $_SESSION['com_id'] ?? '') . "' ");
...
<?php $querycustomer=mysqli_query($db->conn, "select brand_name,brand.id as id from brand join map_type_to_brand on brand.id=map_type_to_brand.brand_id where type_id='" . mysqli_real_escape_string($db->conn, $data_pro['type']) . "'");
...
while($fetch_customer=mysqli_fetch_array($querycustomer)){	?>
          ...
          <div id="box" style="width:18%"><div id="slotmodel[<?=$i?>]"><select id="model[<?=$i?>]" name="model[<?=$i?>]" required class="form-control">
			<?php $querycustomer=mysqli_query($db->conn, "select model_name,id from model where brand_id='" . mysqli_real_escape_string($db->conn, $data_pro['ban_id']) . "' and type_id='" . mysqli_real_escape_string($db->conn, $data_pro['type']) . "'");
...
<textarea name="des[<?=$i?>]" id="des[<?=$i?>]" placeholder="<?=$xml->notes?>"  class="form-control"><?=$data_pro[des];?></textarea></div>
  ...
  <?php $i++;}else
if($mode=="E"){
	$query_pro=mysqli_query($db->conn, "select pro_id,price,type,ban_id,model,quantity,pack_quantity,des,vo_id,DATE_FORMAT(vo_warranty,'%d-%m-%Y') as vo_warranty from product where vo_id='" . mysqli_real_escape_string($db->conn, $_REQUEST['id'] ?? '') . "'");$i=0;
