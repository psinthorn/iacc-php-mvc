...
$query=mysqli_query($db->conn, "select brand.id as id,brand_name from brand join map_type_to_brand on brand.id=map_type_to_brand.brand_id where type_id='".$_GET['value']."'");
...
$query=mysqli_query($db->conn, "select model.id as id,model_name from model ".$condition);
...
$tmp.='<option value="'.$data[id].'">'.$data[model_name].'</option>';           
	...
	$querymodel=mysqli_query($db->conn, "select price,des from model where id='".$_GET[value]."'");
