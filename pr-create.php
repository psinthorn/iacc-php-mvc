...
<form action="core-function.php" method="post" id="company-form">
    ...
    <div id="box">
		...
		<select id="ven_id" name="ven_id" class="form-control">
			...
			if ($com_id > 0) {
			    // Show only vendors this company can work with
			    $querycustomer = mysqli_query($db->conn, "SELECT DISTINCT c.name_en, c.id FROM company c 
			        WHERE c.vender='1' AND c.id != '$com_id' AND c.deleted_at IS NULL
			        ORDER BY c.name_en");
			} else {
...
<div id='box4'>";?>
    ...
    <?php echo"
	<input type='text' name='quantity".$i."'   onchange=\"calprice('".$i."'); \"  required style='float:left; width:15%' class='form-control' id='quantity".$i."' value='1' />
	
	<div style='float:left; padding:1%;'> * </div>
	
	<input type='hidden' name='id".$i."' readonly='true'  id='id".$i."' value='0' />
 ...
 ";}?>
    ...
    <div id="box" style=" float:right"> <?=$xml->summary?> <input type='text' name='totalnet' id='totalnet' value='0' readonly='true' class='form-control' style="margin-bottom:2px;"/>
    </div>
