...
$db->checkSecurity();
 ...
 $query=mysqli_query($db->conn, "select po.name as name,over,ven_id,dis,vat, taxrw as tax2,tax,pr.cus_id as cus_id,payby,des,brandven,valid_pay, DATE_FORMAT(iv.createdate,'%d-%m-%Y') as date,DATE_FORMAT(deliver_date,'%d-%m-%Y') as deliver_date,ref,pic,status from pr join po on pr.id=po.ref  join iv on po.id=iv.tex where po.id='".$_REQUEST['id']."' and status>'2' and (pr.cus_id='".$_SESSION['com_id']."' or ven_id='".$_SESSION['com_id']."') and po_id_new=''");
