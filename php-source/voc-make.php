...
$queryvou=mysql_query("select * from voucher where id='".$_REQUEST[id]."' and vender='".$_SESSION[com_id]."'");
...
<textarea name="des[<?=$i?>]" id="des[<?=$i?>]" placeholder="<?=$xml->notes?>"  class="form-control"><?=$data_pro[des];?></textarea></div>
  ...
  <?php $i++;}else
if($mode=="E"){
	$query_pro=mysql_query("select pro_id,price,type,ban_id,model,quantity,pack_quantity,des,vo_id,DATE_FORMAT(vo_warranty,'%d-%m-%Y') as vo_warranty from product where vo_id='".$_REQUEST[id]."'");$i=0;
